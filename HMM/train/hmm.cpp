...
int N = 10; //states num
int T = 1600; //the length of the sequence
int I = 2000; //the train id num
	...
	while(true){
		...
		for(int j=0; j<N; j++){
			...
			for(int k=0; k<M; k++){
				...
				int q=0;
				while(q<id && exist[q][k]==false) q++;
				if(q<id && exist[q][k] && !exist[id][k])//if the grid was matched before, not matched this time,
					continue;			//its B will be unchanged.
				else
		...
		for(int i=0; i<N; i++){
			for(int k=0; k<M; k++){
				int q=0;
				while(q<id && exist[q][k]==false) q++;
				if(q<id && exist[q][k] && !exist[id][k])//if the grid was matched before,but not matched this time
					continue;			//its B will be unchanged.
				else
					B[i][k] = 0.001 + 0.999 * B[i][k];
			}//end j
		}//end i
...
void trainAll(int train_id, float DELTA){
	printf("In trainAll(): train_id_num = %d, DELTA = %f\n", train_id, DELTA);

	for(int i=0; i<train_id; i++){
		printf("\ntrain_id: %d\n", i);
		
		float log_prob_final = baumWelch(DELTA, train_obs[i], i);
		printMatrices();
		resetLocalVectors();
		printf("\nAfter a round, the log_prob: %f, train_id: %d\n", 
				log_prob_final, i);
	}
	FILE* unzero = fopen("unzero.txt", "w");
	for(int i=0; i<N; i++){
		for(int j=0; j<M; j++){
			fprintf(unzero, "%d %d %f\n", i,j,B[i][j]);
		}
	}
	fclose(unzero);
}
