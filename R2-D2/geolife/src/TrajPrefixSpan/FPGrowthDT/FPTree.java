package TrajPrefixSpan.FPGrowthDT;
...
public class FPTree implements Serializable{
	 ...
	 * 
	 */
	private static final long serialVersionUID = 2544304439071321185L;
	...
	public void addSequence(List<Itemset> sequence){
		...
		addTransaction(seqInt);
	}
	
	
	private int getHight(FPNode sr){
		if(null==sr) return 0;
		int hmax=-1;
		for(FPNode child:sr.childs){
			int ch=getHight(child)+1;
			if(hmax<ch){
				hmax=ch;
			}
		}
	...
	public int getHight(){
		Set<Entry<Integer, FPNode>> mapSet=mapItemNodes.entrySet();
		Iterator<Entry<Integer, FPNode>> mapSetItr=mapSet.iterator();
		int hmax=-1;
		while(mapSetItr.hasNext()){
			Entry<Integer,FPNode> sr=mapSetItr.next();
			
			FPNode start=sr.getValue();
			while(null!=start){
				int srh=getHight(start);
				if(hmax<srh) hmax=srh;
	...
	public void addTransaction(List<Integer> transaction) {
		...
		for(Integer item : transaction){
			...
			if(child == null){ 
				...
				newNode.itemID = item;
				newNode.itemStr="(x:"+(item>>16)+" y:"+((item<<16)>>16)+")";
