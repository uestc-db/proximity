...
public class AlgoAgrawalFaster94 {
		...
		for (int k = 2; k < patterns.getLevels().size(); k++) {
			for (ItemsetApriori lk : patterns.getLevels().get(k)) {
				...
				for (ItemsetApriori hm_P_1 : H1) {
					ItemsetApriori itemset_Lk_minus_hm_P_1 = lk
					...
					if (conf >= minconf) {
						RuleAgrawal rule = new RuleAgrawal(
	...
	private void apGenrules(int k, int m, ItemsetApriori lk,
			Set<ItemsetApriori> Hm) {
		...
		if (k > m + 1) {
			...
			for (ItemsetApriori hm_P_1 : Hm_plus_1) {
				...
				if (conf >= minconf) {
					RuleAgrawal rule = new RuleAgrawal(itemset_Lk_minus_hm_P_1,
							hm_P_1, lk.getAbsoluteSupport(), conf);
					rules.addRule(rule);
	...
	private void calculateSupport(ItemsetApriori itemset_Lk_minus_hm_P_1) {
		...
		for (ItemsetApriori itemset : patterns.getLevels().get(
	 ...
	 * @return
	 */
	protected Set<ItemsetApriori> generateCandidateSizeK(
			Set<ItemsetApriori> levelK_1) {
		...
		for (ItemsetApriori itemset1 : levelK_1) {
			for (ItemsetApriori itemset2 : levelK_1) {
				...
				Integer missing = itemset1
						.allTheSameExceptLastItem(itemset2);
				if (missing != null) {
