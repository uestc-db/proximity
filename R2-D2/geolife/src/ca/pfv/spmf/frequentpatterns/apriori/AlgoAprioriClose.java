...
public class AlgoAprioriClose {
	...
	public Itemsets runAlgorithm(double minsupp) {
		...
		Collection<ItemsetApriori> lastLevel = frequentItemsets.getLevels()
				.get(0); // DIFF
	...
	protected void addEmptySetToLevel0IfNecessary(
			List<ItemsetApriori> candidatesSize1) {
		// We add the emptyset to L0, if no itemset of level 1 has
		// its support equal to the number of transactions in the binary
		// context.
	...
	public void printStats() {
		...
		System.out.println(" Database transactions count : " + context.size());
		...
		Double percentClosed = ((double) frequentClosed.getItemsetsCount() / (double) frequentItemsets
				.getItemsetsCount());
		System.out.println(" Frequent closed itemsets count : "
				+ frequentClosed.getItemsetsCount() + " (" + percentClosed
				+ "% of frequents itemsets )");
